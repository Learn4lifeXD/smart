  ...
  const customReadable = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const surveyorOut = await runAgent(ai, "gemini-2.5-flash", "You are the Surveyor Agent. Analyze topography, zones, and borders concisely in a technical tone.", surveyorInput);
        ...
        const appraiserOut = await runAgent(ai, "gemini-2.5-flash", "You are the Appraiser Agent. Calculate highly realistic budgets, parcel valuations, and financial compensation. Output in millions/billions.", appraiserInput);
        ...
        const auditorOut = await runAgent(ai, "gemini-2.5-pro", "You are the Auditor Agent. Cite specific simulated state codes and expropriation laws. Ensure legal bulletproofing.", auditorInput);

        // 5. Sentinel
        sendUpdate("Sentinel is assessing risk factors...");
        const sentinelInput = `Identify risks for: ${document_text}\nLegal: ${auditorOut}`;
        const sentinelOut = await runAgent(ai, "gemini-2.5-flash", "You are the Sentinel Agent. Assess security, stakeholder friction, media fallout, and operational delays.", sentinelInput);
        ...
        const finalDossier = await runAgent(ai, "gemini-3.5-flash", "You are the Scribe Agent. Synthesize the provided reports into a cohesive, highly professional, classified-style Markdown dossier. Use strict headers, bullet points, and bold text. Start with an Executive Summary.", scribeInput);
